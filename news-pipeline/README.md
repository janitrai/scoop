...
```bash
go run ./cmd/news-pipeline serve --env .env --host 0.0.0.0 --port 8090
```
...
`news-pipeline embed` and `news-pipeline process` call an external HTTP embedding endpoint.
...
Also pass `--triggered-by-topic` to `news-pipeline ingest` for run-level tagging in `ingest_runs`.
...
- OpenClaw calls `news-pipeline ingest`.
...
- Useful flags:
  - `news-pipeline dedup --lookback-days <N>`
  - `news-pipeline process --dedup-lookback-days <N>`
