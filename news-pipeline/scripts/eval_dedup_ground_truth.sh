...
BIN_PATH="${TMPDIR:-/tmp}/news-pipeline-gt-eval"
MOCK_SRC=""
MOCK_LOG=""
MOCK_PID=""
MAP_TSV=""
...
MOCK_SRC="$(mktemp /tmp/news-pipeline-gt-eval-mock-XXXX.go)"
MOCK_LOG="$(mktemp /tmp/news-pipeline-gt-eval-mock-XXXX.log)"
cat > "$MOCK_SRC" <<'GO'
package main
...
MAP_TSV="$(mktemp /tmp/news-pipeline-gt-map-XXXX.tsv)"
ingest_failed=0
...
PRED_TSV="$(mktemp /tmp/news-pipeline-gt-pred-XXXX.tsv)"
PGPASSWORD="$PGPASSWORD" psql -X -h localhost -U news -d news_pipeline -At -F $'\t' <<'SQL' > "$PRED_TSV"
