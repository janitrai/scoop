...
- `news-pipeline ingest --source <name|all>`
- `news-pipeline normalize --limit 1000`
- `news-pipeline embed --model Qwen3-Embedding-8B --model-version v1 --batch-size 32`
- `news-pipeline dedup --lookback-days 365 --limit 2000`
- `news-pipeline assign-topics --limit 2000`
- `news-pipeline digest --topic <slug> --date YYYY-MM-DD [--publish]`
- `news-pipeline run-once --publish` (orchestrates all stages in order)
...
- Operational behavior:
  - one static binary deploy target (`news-pipeline`) for cron and manual runs.
...
- Add worker loop cron every 15 minutes:
  - `*/15 * * * * news-pipeline run-once`
- Add shadow digest jobs at current publish times with `--publish` disabled.

After cutover:
- Keep same topic cron times:
  - `0 9 * * *` -> `news-pipeline digest --topic ai_news --date {{today_berlin}} --publish`
  - `0 9 * * *` -> `news-pipeline digest --topic world_news --date {{today_berlin}} --publish`
  - `0 10 * * *` -> `news-pipeline digest --topic china_news --date {{today_berlin}} --publish`
