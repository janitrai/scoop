...
BIN_PATH="${TMPDIR:-/tmp}/scoop-perf"
MOCK_SRC=""
MOCK_LOG=""
MOCK_PID=""
...
MOCK_SRC="$(mktemp /tmp/scoop-embed-mock-XXXX.go)"
MOCK_LOG="$(mktemp /tmp/scoop-embed-mock-XXXX.log)"
cat > "$MOCK_SRC" <<'GO'
package main
...
decision_counts=$(PGPASSWORD="$PGPASSWORD" psql -X -h localhost -U news -d "$DB_NAME" -At -F '|' <<'SQL'
