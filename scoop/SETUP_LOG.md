...
- Task: Apply schema from `IMPLEMENTATION_PLAN.md` section `1. Complete SQL Schema (DDL)` to `scoop` as user `news`.
- Extracted SQL block to `/tmp/scoop_schema.sql` (348 lines).
- Apply command run:
  - `source .env && PGPASSWORD="$PGPASSWORD" psql -v ON_ERROR_STOP=1 -X -h localhost -U news -d scoop -f /tmp/scoop_schema.sql`
- Apply result:
...
- Task: Bootstrap Go backend implementation for `scoop` (using `~/horse/backend` conventions) and ship first working commands.
...
- Verification executed:
  ...
  - `/tmp/scoop health --env .env` -> `ok: database ping successful`
- Ingest smoke execution:
  - `/tmp/scoop ingest --env .env --source smoke_go_cli --source-item-id go-smoke-1770990053 --payload '{"title":"Go smoke ingest","id":"go-smoke-1770990053"}' --checkpoint '{"cursor":"go-smoke-1770990053"}' --triggered-by-topic setup_smoke`
  - Output included:
...
- Verification executed:
  ...
  - `go vet ./...`
  - `go build -o /tmp/scoop ./cmd/scoop`
  - `/tmp/scoop health --env .env` -> passed
  - Live ingest smoke:
...
- Added new commands in `scoop` CLI:
...
- Live DB modern dedup smoke test:
  ...
  - Ran:
    - `go run ./cmd/scoop process --env .env --normalize-limit 100 --dedup-limit 100 --until-empty=true --max-cycles 5`
  - Process output:
...
- Verification executed:
  ...
  - `go test ./...`
  - `go vet ./...`
  - `go run ./cmd/scoop --help` (includes `embed`)
  - `go run ./cmd/scoop embed -h`
  - `go run ./cmd/scoop process -h`
...
- Task: Align DB layer with `~/horse` conventions and switch `scoop` from `pgx` to GORM.
...
- Runtime UUID prerequisite:
  ...
  - Command:
    - `source .env && PGPASSWORD="$PGPASSWORD" psql -v ON_ERROR_STOP=1 -X -h localhost -U news -d scoop -f migrations/002_add_uuid_columns.sql`

- Verification:
  - `go test ./...` passed.
  - `go run ./cmd/scoop health --env .env` passed.
  - `go run ./cmd/scoop ingest ...` passed with UUID outputs (`run_uuid`, `raw_arrival_uuid`).
  - Temporary smoke rows were deleted after verification.

- API stack note:
  - `scoop` currently has no HTTP API surface (CLI-only), so no Gin-to-Echo migration was required in this repo.
...
- Schema migration added/applied:
  ...
  - Applied via:
    - `source .env && PGPASSWORD="$PGPASSWORD" psql -X -v ON_ERROR_STOP=1 -h localhost -U news -d scoop -f migrations/003_collection_scoped_dedup.sql`
  - Migration changes:
